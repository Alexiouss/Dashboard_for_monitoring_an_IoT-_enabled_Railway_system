document.addEventListener("DOMContentLoaded", () => {
    ...
    const showDataButton_humidity = document.getElementById("Predict_button_humidity");
    const clearButton_humidity = document.getElementById('Clear_button_humidity');
    const Historic_button_humidity = document.getElementById('Historic_button_humidity');
    let chart;
    ...
    async function fetch_humidity() {
        try {
            const response = await fetch("/api/predicted_humidity_tommorrow"); // Replace with your actual API endpoint
            const data = await response.json();
            return data.humidity_values.environment_readings;
        } catch (error) {
    ...
    async function create_humidity_chart() {
        const humidity_data = await fetch_humidity();
        ...
        // Extract humidity from the data
        const humidity = humidity_data.map(reading => reading.humidity);
        ...
        chart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: "Temperature",
                    data: humidity, // Extract 'People' values
    ...
    showDataButton_humidity.addEventListener("click", () => {
        create_humidity_chart();
    });

    let flag=0;
    function update_chart(heating_data_historic) {
        const past_temp=heating_data_historic.humidityvalues;
    ...
    clearButton_humidity.addEventListener('click', async() =>{
        await clearChart();
        flag=0;
    });

    
    Historic_button_humidity.addEventListener('click', async () => {
        try {
    ...
    async function fetch_historic() {
        try {
            const response_historic = await fetch('/api/past_humidity_values'); // This assumes your server is running on the same domain
            if (!response_historic.ok) {
                throw new Error('Failed to fetch data');
            }
            const data_historic = await response_historic.json();
            return data_historic.past_humidity[0]; // Return the entire array
        } catch (error) {
            console.error(error);
